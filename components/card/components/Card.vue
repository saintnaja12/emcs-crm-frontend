<template>
  ...
  >
    <slot></slot>
    <div v-if="loading" class="sc-progress-card" :style="{'margin-top': cssTop }">
...
export default {
    ...
    mounted () {
        this.$nextTick(() => {
            const header = this.$refs.card.querySelector('.sc-card-header-separator');
            if(header) {
                this.cssTop = height(header) / 2  + 'px';
            }
        })
    },
    methods: {

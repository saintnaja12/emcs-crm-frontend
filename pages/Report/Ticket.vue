...
export default {
    ...
    data: () => ({
		...
		pieChartOption: [
			...
			{ text: 'Subject (Top 5)', value: 'subject' },
    ...
    methods: {
		searchTicket(form){
			...
			this.$reportAdapter.getTicketReport(form).then(resp => {
				this.reportTicket = resp.form
			})
		},
		getDataTicketReport(){
			// console.log('hello')
			this.$reportAdapter.getTicketReport().then(resp => {
				this.reportTicket = resp
			})
		},
		pieChartTicket(pieChart){
			this.$reportAdapter.getTicketReport(pieChart).then(resp => {
				this.pieChart = resp
			})
		},
		barChartTicket(barChart){
			this.$reportAdapter.getTicketReport(barChart).then(resp => {
				this.barChart = resp
			})
		}
    },

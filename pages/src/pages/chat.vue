            ...
            >
              <div class="uk-flex uk-flex-column uk-height-1-1 md-bg-grey-200">
                  ...
                  <TransitionGroup id="sc-chat" name="fadeUp" tag="div" class="sc-chat-body sc-light">
                    ...
                    >
                      ...
                      <a href="javascript:void(0)" class="sc-chat-message-avatar">
                        <img v-rjs="userAvatar(group.user, true)" class="sc-avatar" :src="userAvatar(group.user)" alt="">
                      </a>
                ...
                <div class="sc-chat-user-input uk-flex uk-flex-middle">
                  <div class="uk-flex-1">
                    <ScInput v-model.trim="lastMessage.message" placeholder="Write something..." mode="outline" @keyup.native.enter="sendMessage"></ScInput>
                  </div>
                  <a href="javascript:void(0)" class="mdi mdi-send sc-icon-square uk-margin-left" @click.prevent="sendMessage"></a>
                </div>
              </div>
            </div>
                      ...
                      <li v-for="user in users" :key="user.id" class="sc-list-group">
                        <div class="sc-list-addon">
                          <div class="sc-avatar-wrapper">
                            ...
                            <img v-rjs="userAvatar(user.avatar, true)" :src="userAvatar(user.avatar)" class="sc-avatar" alt="">
                          </div>
                        </div>
        ...
        <ul class="uk-list uk-list-divider uk-margin-small-top sc-list-clickable sc-list-align">
          <li v-for="user in users" :key="user.id" class="sc-list-group">
            <div class="sc-list-addon">
              <div class="sc-avatar-wrapper">
                <span class="sc-user-status" :class="[user.status]"></span>
                <img v-rjs="userAvatar(user.avatar, true)"
...
export default {
    ...
    mounted () {
        this.$store.dispatch('setCardFixed', true);
        this.$store.dispatch('setHeaderExpanded', true);
        let group = {
            id: uniqueID(),
            own: false,
            user: 8,
    ...
    beforeDestroy () {
        this.$store.dispatch('setCardFixed', false);
        this.$store.dispatch('setHeaderExpanded', false);
    },
    methods: {
        chatRooms (room) {

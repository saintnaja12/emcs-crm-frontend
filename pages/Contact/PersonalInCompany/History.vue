				...
				<ScCard class="uk-margin-mini-top">
					...
					<ScCardBody style="min-height: 710px">
						...
						<div class="uk-align-right">
							<a class="sc-button sc-button-success sc-button-social" data-uk-toggle="target: #modal-remarkLogPeople">
...
export default {
	...
	created() {
		this.peopleId = this.$route.query.people
	},
	mounted () {
	...
	methods: {
		...
		backPage(){
			this.$router.push(`/Contact/PersonalInCompany/list?company=${this.companyData.id}`)
		},
		getEditHistoryLogCompany(){
			this.$contactAdapter.getHistoryLog(this.peopleId).then(resp => {
				this.historyLogList = _.orderBy(resp.logs, ['timestamp'], ['desc'])
			})
		},
		getRemarkLog(){
			this.$contactAdapter.getHistoryLog(this.peopleId).then(resp => {
				this.$store.dispatch('setRemarkPeopleInCompany', resp.logs)
			})
		},
		formatDate (data) {

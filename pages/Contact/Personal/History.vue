				...
				<ScCard class="uk-margin-mini-top">
					...
					<ScCardBody style="min-height: 710px">
						...
						<div class="uk-align-right">
							<a class="sc-button sc-button-success sc-button-social" data-uk-toggle="target: #modal-remarkLogPersonal">
...
export default {
	...
	created() {
		this.personalId = this.$route.query.personal
	},
	mounted () {
    	this.getEditHistoryLogPersonal()
		this.getRemarkLog()
	},
	methods: {
		...
		backPage(){
			...
			this.$router.push(`/Contact?tabIndex=${personal}`)
		},
		getEditHistoryLogPersonal(){
			let loader = this.$loading.show({
				loader: 'spinner'
			})
			this.$contactAdapter.getHistoryLog(this.personalId).then(resp => {
		...
		getRemarkLog(){
			this.$contactAdapter.getHistoryLog(this.personalId).then(resp => {
				this.$store.dispatch('setPersonalRemarkById', resp.logs)
			})
		},
		formatDate (data) {
